...
if __name__ == "__main__":
    ...
    import json
    from genepro.selection import elitism_selection, tournament_selection, roulette_selection, roulette_selection_elitism, rank_selection
    from genepro.variation import coeff_mutation, subtree_crossover, subtree_mutation

    experiment_name = "rank_selection"
    experiments_dir = "selection_experiments"
    ...
    def hpo_evolve(evo_settings, experiment_name):
        ...
        for i, settings in enumerate(hpo_settings):
            ...
            os.makedirs(f"./{experiments_dir}/{specific_experiment_name}", exist_ok=True)
            with open(f"./{experiments_dir}/{specific_experiment_name}/evo_settings.json", "w") as f:
                serialized_dict = serialize_functions_in_dict(deepcopy(settings))
            ...
            with open(f"./{experiments_dir}/{specific_experiment_name}/evolution_class.pickle", "wb") as f:
                pickle.dump(evo_baseline, f)
            
            generation_evo_fitnesses, generation_test_fitnesses = save_and_evaluate_evo_generations(evo_baseline, fitness_function, specific_experiment_name, num_episodes=5, dir_name=experiments_dir)
            plot_evo_test_fitnesses(generation_evo_fitnesses, generation_test_fitnesses, specific_experiment_name, dir_name=experiments_dir)

    for i in range(n_experiments:=3):
        hpo_evolve(evo_settings, experiment_name=f'{experiment_name}_exp{i}_')